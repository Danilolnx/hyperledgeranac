...
generateCerts.common.enrollIdentity() {
    ...
    fabric-ca-client enroll -u https://$1:$2@ca.${ORG_DOMAIN}:30656 -M $FABRIC_CA_CLIENT_HOME/msp/$1 --tls.certfiles $FABRIC_CA_HOME/tls-cert.pem --csr.hosts "$1"
    retVal=$?
    if [[ $retVal -ne 0 ]]; then
    ...
    fabric-ca-client enroll -u https://$1:$2@ca.${ORG_DOMAIN}:30656 -M $FABRIC_CA_CLIENT_HOME/msp/$1/tls --tls.certfiles $FABRIC_CA_HOME/tls-cert.pem --enrollment.profile tls --csr.hosts "$1"
    retVal=$?
    if [[ $retVal -ne 0 ]]; then
        generateCerts.utils.echoErr "Could not enroll $1 certs, check ca.${ORG_DOMAIN} logs"
        exit 1
...
generateCerts.utils.prepareClientEnvironment() {
    ...
    fabric-ca-client enroll -u https://admin:adminpw@ca.${ORG_DOMAIN}:30656 --tls.certfiles $FABRIC_CA_HOME/tls-cert.pem
    if [[ $? -ne 0 ]]; then
