...
### @generateCerts
## This will generate certificates both for orderer and anacaudit/anacpool certs
## orderer.anacord.gov.br, anacaudit.gov.br, anacpool.gov.br are hard-coded
...
generateCerts() {
    ...
    sleep 5
    local containers=(ca.anacaudit.gov.br ca.anacpool.gov.br)
    local SCRIPTS_FOLDER="/etc/hyperledger/fabric-ca-server/scripts"
    docker exec -it ca.gov.br bash ${SCRIPTS_FOLDER}/generateOrdererCerts.sh
    if [[ $? -ne 0 ]]; then
        return $?
    fi
    docker cp ca.gov.br:/etc/hyperledger/fabric-ca-client/crypto-config ./crypto-config
    if [[ $? -ne 0 ]]; then
        return $?
    fi

    for ca in ${containers[@]}; do
