...
generateOrdererCerts.generate() {
    ...
    local ORDERER_NAME=orderer.anacord.anac.gov.br

    # register it
    local PASSHASH=$(generateCerts.common.registerIdentity ${ORDERER_NAME})
    if [[ ! ${PASSHASH} ]]; then
        generateCerts.utils.echoErr "Could not get password from ${ORDERER_NAME} registration"
        exit 1
    fi
    # enroll it
    generateCerts.common.enrollIdentity $ORDERER_NAME $PASSHASH
    if [[ $? -ne 0 ]]; then
        generateCerts.utils.echoErr "Could not enroll ${ORDERER_NAME} certs, check ca.${ORGANIZATION_DOMAIN} logs"
        exit 1
    fi
...
generateOrdererCerts.createMSPTree() {
    ...
    local CRYPTOFOLDER=crypto-config/ordererOrganizations/${ORGANIZATION_DOMAIN}
